...
namespace Doppler.PushContact.Services
{
    public class PushContactService : IPushContactService
    {
        ...
        private bool IsValidSubscription(SubscriptionDTO subscription)
        {
        ...
        public async Task AddAsync(PushContactModel pushContactModel)
        {
            ...
            if (string.IsNullOrEmpty(pushContactModel.DeviceToken))
            {
                throw new ArgumentException($"{nameof(pushContactModel.DeviceToken)} is not valid");
            }

            // If the subscription is valid, and possibly a hashed token from it, there's no need to validate the token through Firebase.
            // However, if the subscription is invalid, the token must be one generated by Firebase, and validated through it.
            // TODO: remove token validation with Firebase, when Firebase will be permanently removed.
            if (!IsValidSubscription(pushContactModel.Subscription) && !await _deviceTokenValidator.IsValidAsync(pushContactModel.DeviceToken))
            {
                throw new ArgumentException(
                    $"{nameof(pushContactModel.DeviceToken)} and {nameof(pushContactModel.Subscription)} are not valid"
                );
            }
