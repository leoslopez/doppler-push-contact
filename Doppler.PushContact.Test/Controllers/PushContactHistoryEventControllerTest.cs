...
namespace Doppler.PushContact.Test.Controllers
{
    public class PushContactHistoryEventControllerTest : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        public async Task BulkAdd_should_return_unauthorized_when_token_is_not_valid(string token)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "history-events/_bulk")
            {
        ...
        public async Task BulkAdd_should_require_a_valid_token_with_isSU_flag(string token)
        {
            // Arrange
            var client = _factory.CreateClient(new WebApplicationFactoryClientOptions());
